...
import { initPhotoAssets, photoAssetsReady, generatePhotoHTML, getRoleSizeMultiplier } from './people-photos.js';
import {
...
const MIXED_WEIGHTS = { Student: 0.75, Teacher: 0.25 };
...
function renderFigures() {
  ...
  layer.innerHTML = people.map((p) => {
    ...
    if (figureStyle === 'photo' && photoAssetsReady()) {
      ...
      effectiveSize = size * getRoleSizeMultiplier(p.role);
    } else {
    ...
    if (isLegacy) {
      ...
      return `
        <div class="figure-marker${legacyClass}" style="
          left: ${p.position.x * 100}%;
          top: ${p.position.y * 100}%;
          width: ${effectiveSize}px;
          height: ${effectiveSize * 1.15}px;
